from NumbER.matching_solutions.gaussian.dataset import PairBasedDataset
...
class GaussianDataModule(pl.LightningDataModule):
    def __init__(self, train_data: pd.DataFrame, valid_data: pd.DataFrame, test_data: pd.DataFrame, predict_data: pd.DataFrame, batch_size: int = 50):
        ...
        self.train_dataset = self.dataset_class(self.train_data['all_data'], self.train_data['numerical_feature_vector'], self.train_data['textual_data'], self.train_data['matches'])
        self.valid_dataset = self.dataset_class(self.valid_data['all_data'], self.valid_data['numerical_feature_vector'], self.valid_data['textual_data'], self.valid_data['matches'])
        self.test_dataset = self.dataset_class(self.test_data['all_data'], self.test_data['numerical_feature_vector'], self.test_data['textual_data'], self.test_data['matches'])
        self.predict_dataset = self.dataset_class(self.predict_data['all_data'], self.predict_data['numerical_feature_vector'], self.predict_data['textual_data'], self.predict_data['matches'])
