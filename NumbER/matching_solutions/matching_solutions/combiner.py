from NumbER.matching_solutions.combiner.combiner import Combiner
from NumbER.matching_solutions.combiner.data_loader import CombinerDataModule
...
class CombinerMatchingSolution(MatchingSolution):
	...
	def model_train(self, l_epochs, l_params, x_epochs, x_params, wandb_id, include_numerical_features_in_textual, seed, textual_config, finetune_batch_size, num_finetune_epochs, train_goldstandard_path, valid_goldstandard_path, test_goldstandard_path,i, lr=3e-5):
		...
		lightgbm_test = self.lightgbm.model_predict(lightgbm_model)['predict'][0]['score']
		xgboost_test = self.xgboost.model_predict(xgboost_model)['predict'][0]['score']
		train_combined = np.reshape([lightgbm_train, xgboost_train], newshape=(len(lightgbm_train), 2))
		valid_combined = np.reshape([lightgbm_valid, xgboost_valid], newshape=(len(lightgbm_valid), 2))
		test_combined = np.reshape([lightgbm_test, xgboost_test], newshape=(len(lightgbm_test), 2))
		combiner = Combiner(
         	textual_component=textual_component,
			learning_rate=lr,
        )		
		self.train_data = self.process_dataframe(train_record_data, train_goldstandard_data, textual_finetune_formatter, train_combined, include_numerical_features_in_textual, train_record_data)
		self.valid_data = self.process_dataframe(valid_record_data, valid_goldstandard_data, textual_finetune_formatter, valid_combined, include_numerical_features_in_textual, train_record_data)
		self.test_data = self.process_dataframe(test_record_data, test_goldstandard_data, textual_finetune_formatter, test_combined, include_numerical_features_in_textual, train_record_data)
		self.data = CombinerDataModule(train_data=self.train_data, valid_data=self.valid_data, test_data=self.test_data, predict_data=self.test_data, batch_size=finetune_batch_size)
		checkpoint_callback_1 = ModelCheckpoint(
					monitor="val_loss",
					mode="min",
					save_top_k=1,
					dirpath="/hpi/fs00/share/fg-naumann/lukas.laskowski/saved_models/combiner/",
		...
		trainer.fit(combiner, self.data)
		#add xgboost and lightgbm
		combiner = combiner.load_from_checkpoint(checkpoint_callback_1.best_model_path,learning_rate=lr, textual_component=textual_component)
		return None, combiner, None, None
	...
	def model_predict(self, model, train_goldstandard_path, valid_goldstandard_path, test_goldstandard_path):
		...
		valid = CombinerDataModule(train_data=self.train_data, valid_data=self.valid_data, test_data=self.test_data, predict_data=self.valid_data)
		...
		path = "/hpi/fs00/share/fg-naumann/lukas.laskowski/saved_models/combiner"
	...
	def process_dataframe(self, df: pd.DataFrame, matches: pd.DataFrame, textual_formatter, statistical_output, include_numerical_features_in_textual=True, train_data=None):
		...
		return {'all_data': df, 'textual_data': textual_data, 'statistical_output': statistical_output, 'matches': matches}
