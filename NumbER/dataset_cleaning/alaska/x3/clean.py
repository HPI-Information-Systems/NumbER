...
X['core_refine']=""
for i,item in X.iterrows():
    core_strs= (' '.join([str(item['cpu_brand']),str(item['cpu_type']),str(item['title'])])).lower()
...
X['frequency_refine']=""
for i,item in X.iterrows():
    arow=(str(item['title'])+" "+str(item['cpu_frequency'])).lower().replace('ghz.',"ghz ")
    if 'ghz' in arow:
...
X['storage_refine']=""
for i,item in X.iterrows():
    row=(str(item['title'])+" " + str(item['ram_capacity'])+" "+str(item['ssd_capacity'])).lower().replace("("," ").replace(" gb","gb").replace("g ","gb ")
...
X['ram_refine']=""
for i,item in X.iterrows():
    row=(str(item['title'])+" " + str(item['ram_capacity'])).lower().replace("("," ").replace(" gb","gb").replace("g ","gb ")
    for one in ram_list:
        if one in row:
...
#X.loc[X['frequency_refine'] == 'i5-4200u-1.6ghz-13.3""', "frequency_refine"] = '1.6ghz'
pattern = re.compile(r'(\d+(\.\d+)?)(?=ghz)')
X['frequency_refine'].str.extract(r'(\d+(\.\d+)?)(?=ghz)').iloc[:,0].to_csv('temp.csv',index=False)

#X['frequency_refine'].str.replace(pattern,regex=True).to_csv('temp.csv',index=False)
X['frequency_refine'] = X['frequency_refine'].str.extract(r'(\d+(\.\d+)?)(?=ghz)').iloc[:,0].replace("", np.nan).astype(float)
