...
datasets = [ "dblp_acm", "dblp_acm_dirty", "dblp_scholar", "dblp_scholar_dirty", "fodors_zagat", "itunes_amazon", "itunes_amazon_dirty", "walmart_amazon", "walmart_amazon_dirty"]
datasets = [ "company"]
...
for dataset in tqdm(datasets):
    ...
    output_folder = f"/hpi/fs00/share/fg-naumann/lukas.laskowski/datasets/dm_{dataset}/"
    ...
    gold_standard_old = pd.concat([pd.read_csv(f"{base_path}/test.csv"),pd.read_csv(f"{base_path}/train.csv"), pd.read_csv(f"{base_path}/valid.csv")])#.reset_index()
