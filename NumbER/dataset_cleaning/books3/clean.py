...
all_data = pd.concat([pd.read_csv('./books3/csv_files/barnes_and_noble.csv'), pd.read_csv('./books3/csv_files/half.csv')]).drop(columns=['ID'])
all_data['id'] = all_data.index
all_data.to_csv('./books3_all_features.csv', index=False)
...
df['id'] = df.index
df.to_csv('./books3_features.csv', index=False)
gold_standard.to_csv('./books3_matches.csv', index=False)
matches = pd.read_csv('./books3_matches.csv').rename(columns={'left_instance_id': 'p1', 'right_instance_id': 'p2', 'label': 'prediction'})
matches = matches[matches['prediction'] == 1]
matches.to_csv('./books3_matches.csv', index=False)
